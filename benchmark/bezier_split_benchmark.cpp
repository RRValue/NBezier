...
static void BM_BezierSplitRunTimeDegree8_new(benchmark::State& state)
{
    constexpr auto p0 = Point<float, 6>{-4, -4, -4, -4, -4, -4};
    constexpr auto p1 = Point<float, 6>{-3, -3, -3, -3, -3, -3};
    constexpr auto p2 = Point<float, 6>{-2, -2, -2, -2, -2, -2};
    ...
    for(auto _ : state)
    {
        benchmark::DoNotOptimize(split_result = BezierSplit::at2(bezier, 0.5f));
    }
}
...
BENCHMARK(BM_BezierSplitRunTimeDegree8_new);